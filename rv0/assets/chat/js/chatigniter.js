...
$(document).on('click', '[data-toggle="popover"]', function(){
        ...
        userkey = $(this).find('input[name="user_key"]').val();
        ...
        var top = ($('#chat-box').is(':visible') ? (offset.top - h_title - 40) : (offset.top + h_title - 20));console.log(h_main+'----'+$('#chat-box').height());
        if((top + $('#chat-box').height()) > h_main){ top = h_main -  $('#chat-box').height();}
        $('#chat-box').css({'top': top-51});
...
$(document).on('keypress', '.chat-textarea input', function(e){
        ...
        if(message !== "" && e.which == 13){
            txtarea.val('');
            // save the message 
            $.ajax({ type: "POST", url: base  + "chat/save_message", data: {message: message, user : user,user_key : userkey},cache: false,
    ...
    $.ajax(
        {
            ...
            success: function(data){
                if(data.success){
					...
					$("#chat_count").html(totalcount);
					var tcount = $("#chat_count").html();
					 if(tcount!=0){
						 $("#menu-toggle").find( "i" ).addClass('animation-chat');
					 }
...
function load_thread(user, limit){
       ...
       $.ajax({ type: "POST", url: base  + "chat/messages", data: {user : user,user_key : userkey, limit:limit },cache: false,
        ...
        if(response.success){
            ...
            if(buddy.more){
             $('ul.chat-box-body').append('<li id="load-more-wrap" style="text-align:center"><a onclick="javascript: load_thread(\''+buddy.id+'\', \''+buddy.limit+'\')" class="btn btn-xs btn-info" style="width:100%">View older messsages('+buddy.remaining+')</a></li>');
              ...
              //var tcount = $("#chat_count").html();
              //console.log(response.unreadcount+'----'+tcount); 
              var bcount=response.totalunreadcount-response.unreadcount;
             $("#chat_count").html(bcount); 
             var tcount = $("#chat_count").html();
			 if(tcount==0){
				 $("#menu-toggle").find( "i" ).removeClass('animation-chat');
			 }
            }
        }});
}
