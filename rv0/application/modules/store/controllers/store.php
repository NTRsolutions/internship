...
class Store extends MX_Controller {
	public function __construct()
        ...
        function store_collection($name)
        {
            $d=$this->Mdl_store->getcollection($name);
			...
			$d['collection']=$c;
			$a=$this->Mdl_store->get_c_products($p);
			$d['products']=$a;
			
            $data['theme_body']=$this->load->view('store_collection',$d);
            $this->load->view('theme/gj/inner_layout', $data);
        }
    ...
    public function add_to_cart() {
       ...
       // echo "hi";exit;
        ...
        if($quantity!=0)
        {
            ...
            {
            		$session_id=get_cookie('rightlink_cart');
                    $cart=array("session_id"=>$session_id ,"user_id"=>$layout_session['user']['id'],$column=>$value,"quantity"=>$quantity,"tenant_id"=>TENENT_ID);
                   // print_r($cart);
            ...
            else
            {
            	if(!empty(get_cookie('rightlink_cart')))
            	{
            		...
            		$session_id=get_cookie('rightlink_cart');
            		$cart=array("session_id"=>$session_id ,"user_id"=>'',$column=>$value,"quantity"=>$quantity,"tenant_id"=>TENENT_ID);
...
{
    $d['collections']=$this->Mdl_store->getstandardproducts();
...
function store_custom()
 {
   ...
   // $this->load->view('store_product',$p);
    ...
    {   
        ...
        $this->Mdl_store->update_cart($pid,$q);
