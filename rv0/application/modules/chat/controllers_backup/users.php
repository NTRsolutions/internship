...
class Users extends CI_Controller {
	...
	public function index()
	{
		...
		//get all admin users
		$adminuser = $this->user->get_adminUsers();
		foreach ($adminuser as $key=>$auser) {
			//get unread messages from this user
			$unread = $this->message->unread_per_user($id, $auser->id);
			$adminuser[$key]->unread =  $unread > 0 ? $unread : null ; 
		}
		//get all alumnis
		$contacts = $this->user->get_allusers($id);
	...
	{
		 //echo $this->input->post('userdata');exit;
		$session = $this->session->userdata('logged_in');
		$id = $session['user']['id'];
		//$data['cur_user'] = $this->user->get($id);
		/*get all admin users
		...
		//get all alumnis
		$contacts = $this->user->get_allusers_search($id,$this->input->post('userdata'));
		...
		$searchData=json_encode($contacts);
		echo $searchData;exit;
